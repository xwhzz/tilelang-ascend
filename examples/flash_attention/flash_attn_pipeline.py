...
def flash_attention_fwd(
    ...
    def main(
            ...
            workspace_1: T.Tensor([block_num, stages, block_M, block_N], accum_dtype),
            workspace_2: T.Tensor([block_num, stages, block_M, block_N], dtype),
            workspace_3: T.Tensor([block_num, stages, block_M, dim], accum_dtype),
    ):
        with T.Kernel(block_num, is_npu=True) as (cid, vid):
            ...
            with T.Scope("C"):
                for i in range(stages):
                    T.set_cross_flag("M", i + stages * 3)

                T.copy(Q[bz, by, bx * block_M:(bx + 1) * block_M, :], q_l1)
                T.barrier_all()
                for kk in T.serial(T.ceildiv(seq_len, block_N * stages)):
                    for i in range(stages):
                        k = kk * stages + i
                        ...
                        T.barrier_all()
                        T.set_cross_flag("FIX", i + stages * 1)

                    for i in range(stages):
                        k = kk * stages + i
                        T.wait_cross_flag(i + stages * 2)
                        T.copy(workspace_2[cid, i, :, :], acc_s_l1)
                        T.barrier_all()
                        T.set_cross_flag("MTE2", i + stages * 3)
            ...
            with T.Scope("V"):
                for i in range(stages):
                    T.set_cross_flag("V", i)
                    T.set_cross_flag("V", (i + stages * 4))
                T.fill(acc_o, 0.0)
                T.fill(sumexp, 0.0)
                T.fill(m_i, -2**30)
                T.barrier_all()

                for _k in T.serial(T.ceildiv(seq_len, block_N * stages)):
                    for i in range(stages):
                        T.fill(acc_s_ub, 0.0)
                        T.barrier_all()
                        if i == 0:
                        ...
                        T.copy(
                            workspace_1[cid, i, vid * block_M // 2:vid * block_M // 2 + block_M // 2, :],
                            acc_s_ub_)
                        T.barrier_all()
                        T.set_cross_flag("MTE2", i)
                        ...
                        T.copy(
                            acc_s_half,
                            workspace_2[cid, i, vid * block_M // 2:vid * block_M // 2 + block_M // 2, :])
                        T.barrier_all()
                    ...
                    for i in range(stages):
                        ...
                        T.copy(
                            ...
                            acc_o_ub)
                        T.barrier_all()
                        T.set_cross_flag("MTE2", i + stages * 4)
...
workspace_1 = torch.zeros((block_num, stages, 64, 64), dtype=torch.float)
workspace_2 = torch.zeros((block_num, stages, 64, 64), dtype=torch.float16)
workspace_3 = torch.zeros((block_num, stages, 64, 512), dtype=torch.float)
